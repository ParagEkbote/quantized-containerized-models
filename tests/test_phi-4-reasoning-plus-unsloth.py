...
def test_predictor_setup_and_predict_cpu(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        predictor.FastLanguageModel, "from_pretrained", staticmethod(_fake_from_pretrained)
    )
    ...
    out_path_obj = pred.predict(
        prompt="test", max_new_tokens=5, temperature=0.5, top_p=0.9, seed=999
    )
...
@pytest.mark.parametrize(
    "cuda_available,bf16_supported,expected_dtype",
    [
...
def test_predictor_generate_params(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        predictor.FastLanguageModel, "from_pretrained", staticmethod(fake_from_pretrained)
    )
    monkeypatch.setattr(torch.cuda, "is_available", lambda: False)
    monkeypatch.setattr(
        predictor, "save_text", lambda folder, seed, index, text: tmp_path / "out.txt"
    )
